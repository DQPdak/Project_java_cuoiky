...
import ut.edu.auth.dto.request.ChangePasswordRequest;                  // DTO: dữ liệu request đổi mật khẩu (oldPassword, newPassword, v.v.)
import ut.edu.auth.dto.response.UserResponse;                          // DTO: dữ liệu trả về cho client (ẩn thông tin nhạy cảm, định dạng trả về)
import ut.edu.auth.entity.User;                                        // Entity User: biểu diễn người dùng trong hệ thống (bảng DB)
import ut.edu.auth.exception.InvalidCredentialsException;              // Exception tuỳ chỉnh: ném khi thông tin xác thực không hợp lệ (mật khẩu cũ sai)
import ut.edu.auth.exception.UserNotFoundException;                    // Exception tuỳ chỉnh: ném khi không tìm thấy người dùng
import ut.edu.auth.repository.UserRepository;                          // Repository: thao tác với DB cho User (findByEmail, save, delete, ...)
