...
export default function MyApplicationsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDING': return <span className="flex items-center text-yellow-600 bg-yellow-50 px-3 py-1 rounded-full text-sm"><Clock size={14} className="mr-1"/> Đang chờ duyệt</span>;
    ...
    <div className="max-w-5xl mx-auto py-8 px-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl shadow-sm border">
          <p className="text-gray-500 mb-4">Bạn chưa ứng tuyển công việc nào.</p>
          <Link href="/dashboard-candidate" className="text-blue-600 hover:underline">Tìm việc ngay</Link>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {applications.map((app: any) => (
                <tr key={app.id} className="hover:bg-gray-50 transition">
                  <td className="p-4 font-medium text-blue-600">{app.jobTitle}</td>
